...
from config import format_pay_header, paymentmethod
...
def automateLogin(STUDENT_ID: str, PASSWORD: str, GYM_ID: str, ITEM_ID: str):
    ...
    courtBookLink = driver.find_element(By.XPATH, "//a[@href='/gymsite/gymBookViewAction.do?ms=viewBook_site&fromPage=index']")
    courtBookLink.click()

    bookCourtLink = driver.find_element(By.XPATH, "//a[@href='javascript:viewBook()']")
    bookCourtLink.click()

    confirmButton = driver.find_element(By.XPATH, "//button[contains(@onclick,\"saveAttentionState()\")]")
    driver.execute_script("arguments[0].click();", confirmButton)

    qimoButton = driver.find_element(By.XPATH, f"//a[@href='/gymbook/gymBookAction.do?ms=viewGymBook&gymnasium_id={GYM_ID}&item_id=&time_date=&userType=']")
    qimoButton.click()

    curDate = str(datetime.date.today())
    curQimoBadmintonButton = driver.find_element(By.XPATH, f"//a[@href='/gymbook/gymBookAction.do?ms=viewGymBook&gymnasium_id={GYM_ID}&item_id={ITEM_ID}&time_date={curDate}&userType=']")
    ...
    username = driver.find_element(By.XPATH, "//div[@class='member-box round-all']//strong").text

    return serverid, jsessionid, username, driver

def automatePay(driver, session, serverid, jsessionid):
    orderButton = driver.find_element(By.XPATH, "//a[@href='/pay/payAction.do?ms=getOrdersForNopay']")
    orderButton.click()

    if paymentmethod == 1:
        payNowButton = driver.find_element(By.XPATH, "//a[contains(@onclick, \"payNow\")]")
        ...
        submitButton = driver.find_element(By.XPATH, "//a[contains(@onclick,\"savePayFrmWithPay()\")]")
        ...
        alipayButton = driver.find_element(By.XPATH, "//img[@src='/zjjsfw/img/zfb.png']")
        alipayButton.click()
